...
import {createAssessment} from '@/service/interface/assessment';
...
const CreateData = ({ setDialogStatus, reloadPage }) => {
  ...
  const _onFinish = async (data) => {
    ...
    let { code, msg } = await createAssessment(data);
    if (!code) {
      message.success(msg);
      reloadPage();
  ...
  const groupData = {
    ...
    date: () => (
      <DatePicker placeholder="please choose enroll date" style={{ width: '100%' }} />
    ),
  };
